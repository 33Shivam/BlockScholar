...
contract Staff {
    ...
    function resultNpay(
        ...
        //Add to array
        result.push(
            Result({ID: _ID, Attendance: _Attendance, AvgMark: _AvgMark})
        );
    ...
    function processRefund(uint256 _ID) public payable {
        //Get required info from Scholarship contract
        ScholarDetails sc = ScholarDetails(SchlContractAdd);
        ...
        require(
            keccak256(abi.encodePacked(status)) ==
                keccak256(abi.encodePacked("pending_refund")),
